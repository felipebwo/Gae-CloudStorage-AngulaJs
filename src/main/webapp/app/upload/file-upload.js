...
(function() {
					...
					[
							...
							function($http, $q, $timeout) {
								...
								this.upload = function(config) {
									...
									config.transformRequest = config.transformRequest ? (angular
											.isArray(config.transformRequest) ? config.transformRequest
									...
									config.transformRequest
											.push(function(data) {
												...
												if (config.file != null) {
													...
													if (angular
															.isArray(config.file)) {
														var isFileFormNameString = angular
																.isString(fileFormName);
	...
	function linkFileSelect(scope, elem, attr, ngModel, $parse, $timeout,
		...
		var changeFnAttr = attr.ngfChange
				|| (attr.ngfSelect && attr.ngfSelect.indexOf('(') > 0);
			...
			if (!isUpdating) {
				...
				try {
					...
					updateModel($parse, $timeout, scope, ngModel, attr,
							changeFnAttr, files, rejFiles, evt);
					if (files.length == 0)
		...
		function bindAttrToFileInput(fileElem) {
			...
			if (attr.ngfDisabled)
				fileElem.attr('disabled', $parse(attr.ngfDisabled)(scope));
		...
		function createFileInput(evt) {
			...
			if (isInputTypeFile()) {
				...
				fileElem.css('display', 'none').attr('tabindex', '-1').attr(
						'__ngf_gen__', true);
		...
		function resetModel(evt) {
			updateModel($parse, $timeout, scope, ngModel, attr, changeFnAttr,
					[], [], evt, true);
		}
		...
		if (window.FileAPI && window.FileAPI.ngfFixIE) {
			window.FileAPI.ngfFixIE(elem, createFileInput, bindAttrToFileInput,
					changeFn, resetModel);
		} else {
			elem.bind('click touchend', clickHandler);
		}
	...
	function linkDrop(scope, elem, attr, ngModel, $parse, $timeout, $location) {
		...
		if (attr.dropAvailable) {
			$timeout(function() {
				scope[attr.dropAvailable] ? scope[attr.dropAvailable].value = available
		...
		elem[0].addEventListener('drop', function(evt) {
			...
			extractFiles(evt,
					function(files, rejFiles) {
						updateModel($parse, $timeout, scope, ngModel, attr,
								attr.ngfChange
										|| (attr.ngfDrop && attr.ngfDrop
												.indexOf('(') > 0), files,
								rejFiles, evt)
					}, $parse(attr.ngfAllowDir)(scope) != false, attr.multiple
							|| $parse(attr.ngfMultiple)(scope));
		}, false);
	...
	function validate(scope, $parse, attr, file, evt) {
		...
		return (val == null || match)
				&& (file.size == null || (file.size < fileSizeMax && file.size > fileSizeMin));
	}
